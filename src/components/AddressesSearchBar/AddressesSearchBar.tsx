...
import { useSearchAddresses } from "../../hooks/customers/address/useSearchAddresses";
import {
  Container,
  List,
  ListItem,
...
export const AddressesSearchBar: FC<AddressesSearchBarProps> = ({
  ...
  const selectOptionHander = (address: CustomerSecondaryAddress) => {
    setSearchTerm("");
    setSuggestionsOpen(false);
    onSelected(address);
  };
          ...
          <List>
            {error ? (
              ...
              addresses?.map((address) => (
                <ListItem key={`${address.customerId}_${address.id}`}>
                  <ListItemButton onClick={() => selectOptionHander(address)}>
                    <ListItemText
